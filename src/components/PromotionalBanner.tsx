  ...
  const renderBannerContent = () => {
    ...
    if (currentBanner.content_type === 'job' && jobDetails) {
      return <div className="flex justify-center items-center flex-1">
          <div className={`relative flex flex-col gap-1.5 w-full md:w-full lg:w-[78%] xl:w-[47%] p-2.5 pt-4 pb-2.5 border-2`} style={jobDetails.is_exclusive ? {
          ...
          borderRadius: '1rem'
            ...
            <div className="flex-1 space-y-0.5 -mt-1">
              ...
              <div className="flex flex-wrap items-center gap-2 pr-20 md:pr-4">
                ...
                {jobDetails.is_exclusive && <Badge className="bg-gradient-to-r from-purple-500 via-blue-500 to-cyan-500 text-white border-0 shrink-0">
                    HuntoriX Exclusive
                  </Badge>}
              </div>
              ...
              {(jobDetails.employment_type || jobDetails.seniority) && <div className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              {/* Description */}
              {jobDetails.description && <p className="text-sm text-muted-foreground line-clamp-2">
                  {jobDetails.description}
                </p>}

              {/* Meta information - compact grid */}
              <div className="grid grid-cols-2 gap-x-4 gap-y-0.5 text-sm text-muted-foreground">
                {jobDetails.location && <div className="flex items-center gap-2">
                    <MapPin className="h-4 w-4 shrink-0" />
                    <span className="truncate">{jobDetails.location}</span>
                  </div>}
                
                {(jobDetails.budget_min || jobDetails.budget_max) && <div className="flex items-center gap-2">
