...
const JobDetail = () => {
  ...
  const checkSaved = async () => {
    ...
    const {
      data,
      error
    } = await supabase.from('saved_jobs').select('id').eq('user_id', user.id).eq('job_id', id).maybeSingle();
    if (error) {
  ...
  const fetchJob = async () => {
    try {
      const {
        data: jobData,
        error: jobError
      } = await supabase.from('jobs').select('*, employer:profiles!jobs_created_by_fkey(*)').eq('id', id).single();
      ...
      if (user && profile?.role === 'headhunter') {
        const {
          data: appData
        } = await supabase.from('applications').select('id, status').eq('job_id', id).eq('headhunter_id', user.id).maybeSingle();
        setHasApplied(!!appData);
        // Store the application status for display
        if (appData) {
          setJob((prev: any) => ({
            ...prev,
            userApplicationStatus: appData.status
          }));
        }
        ...
        const {
          data: inviteData
        } = await supabase.from('job_invitations').select('id, status').eq('job_id', id).eq('headhunter_id', user.id).eq('status', 'pending').maybeSingle();
      ...
      if (user && jobData?.created_by === user.id) {
        const {
          data: appsData,
          error: appsError
        } = await supabase.from('applications').select('*, headhunter:profiles!applications_headhunter_id_fkey(*)').eq('job_id', id).order('created_at', {
          ascending: false
        });
  ...
  return <div className="min-h-screen bg-gradient-to-b from-background via-[hsl(var(--surface))] to-background">
      ...
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <div className="grid lg:grid-cols-[2fr,380px] gap-8 lg:gap-12">
          <div className="space-y-6 min-w-0 mx-auto w-full max-w-3xl lg:max-w-none lg:mx-0">
                ...
                <div className="flex items-start justify-between gap-4 mb-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      {job.job_id_number && <Badge variant="outline" className="text-sm">
                          #{job.job_id_number}
                        </Badge>}
                    </div>
                    <div className="flex flex-wrap gap-2 mb-2">
                      {job.is_exclusive && <Badge className="bg-gradient-to-r from-purple-500 via-blue-500 to-cyan-500 text-white border-0 font-semibold">
                          HuntoriX Exclusive
                        </Badge>}
                      
                      {job.industry && <Badge style={{
                      backgroundColor: getIndustryColor(job.industry)
                  ...
                  <TooltipProvider delayDuration={0}>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button variant="ghost" size="icon" className="h-10 w-10" onClick={handleSaveToggle} disabled={savingJob}>
                          <Heart className={`h-5 w-5 transition-colors ${isSaved ? 'fill-[hsl(var(--accent-pink))] text-[hsl(var(--accent-pink))]' : ''}`} />
                        </Button>
                      </TooltipTrigger>
                      <TooltipContent>
            ...
            <Card className="rounded-2xl">
              ...
              <CardContent className="space-y-4">
                <div>
                  ...
                  {job.employer?.company_sector && <p className="text-sm text-muted-foreground">{job.employer.company_sector}</p>}
                </div>
                
                {job.employer?.bio && <p className="text-sm text-muted-foreground">{job.employer.bio}</p>}

                {job.employer?.company_size && <div>
                    <p className="text-xs text-muted-foreground">Company Size</p>
                    <p className="text-sm font-medium">{job.employer.company_size}</p>
                  </div>}

                {job.employer?.company_hq && <div>
                    <p className="text-xs text-muted-foreground">Headquarters</p>
                    <p className="text-sm font-medium">{job.employer.company_hq}</p>
                  </div>}

                {job.employer?.website && <Button variant="outline" size="sm" className="w-full" onClick={() => window.open(job.employer.website, '_blank')}>
                    Visit Website
                  </Button>}
              </CardContent>
            </Card>

            {profile?.role === 'headhunter' && <Card className="rounded-2xl">
                <CardContent className="pt-6">
                  {hasApplied ? <div className="text-center space-y-2">
                      <Badge className={job?.userApplicationStatus === 'rejected' ? 'bg-[hsl(var(--destructive))] text-white' : job?.userApplicationStatus === 'selected' ? 'bg-[hsl(var(--success))] text-white' : job?.userApplicationStatus === 'shortlisted' ? 'bg-[hsl(var(--accent-lilac))] text-white' : 'bg-[hsl(var(--success))] text-white'}>
                        {job?.userApplicationStatus === 'rejected' ? 'Rejected' : job?.userApplicationStatus === 'selected' ? 'Selected' : job?.userApplicationStatus === 'shortlisted' ? 'Shortlisted' : 'Applied'}
                      </Badge>
                      <p className="text-sm text-muted-foreground">
                        {job?.userApplicationStatus === 'rejected' ? 'Your application was not successful for this position' : 'You have already applied to this job'}
                      </p>
                    </div> : job.status === 'pending_review' ? <TooltipProvider delayDuration={0}>
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <div>
                            <Button variant="outline" size="lg" className="w-full" disabled>Under Review</Button>
                          </div>
                        </TooltipTrigger>
                        <TooltipContent>
                    ...
                    </TooltipProvider> : hasInvitation ? <Button variant="hero" size="lg" className="w-full" onClick={() => navigate(`/job-review/${id}`, {
                state: {
                  from: 'job',
                  parentFrom: fromSource
                }
              })}>
                      Review Invitation
                    </Button> : <Button variant="hero" size="lg" className="w-full" onClick={handleApply}>
                      Apply Now
                    </Button>}
                </CardContent>
              </Card>}
              ...
              <div className="flex items-center justify-between">
                ...
                <TooltipProvider delayDuration={0}>
                  <Tooltip>
                    ...
                    <TooltipContent>
                      <p className="max-w-sm">Our AI analyzes each headhunter&apos;s success rate and performance data to help you choose the best match</p>
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
              </div>
                ...
                {applications.map(app => <Card key={app.id}>
                    ...
                    <CardContent>
                      <div className="space-y-3">
                        {app.cover_note && <div className="text-sm">
                        ...
                        {app.status === 'submitted' && <div className="flex gap-2">
                            <Button size="sm" variant="default" onClick={() => handleAccept(app.id, app.headhunter_id)}>
                              <Check className="mr-2 h-4 w-4" />
                              Accept
                            </Button>
                            <Button size="sm" variant="destructive" onClick={() => handleDecline(app.id, app.headhunter_id)}>
                              <X className="mr-2 h-4 w-4" />
                              Decline
                            </Button>
                            <Button size="sm" variant="outline" onClick={() => handleChat(app.headhunter_id)}>
                              <MessageCircle className="mr-2 h-4 w-4" />
                              Chat
                            </Button>
                          </div>}
                        
                        {app.status !== 'submitted' && <Button size="sm" variant="outline" onClick={() => handleChat(app.headhunter_id)}>
                            <MessageCircle className="mr-2 h-4 w-4" />
                            Chat
                          </Button>}
                      </div>
